...
int iniciar_servidor(t_log* logger, const char* name, char* ip, char* puerto) {
    ...
    // Aviso al logger
    log_info(logger, "Escuchando en IP:%s , PUERTO:%s, (MODULO %s)\n", ip, puerto, name);
...
int esperar_cliente(t_log* logger, const char* name, int socket_servidor) {
    ...
    log_info(logger, "Cliente conectado (MODULO %s) \n ", name);
...
int crear_conexion(t_log* logger, const char* server_name, char* ip, char* puerto) {
    ...
    if(socket_cliente == -1) {
        log_error(logger, "Error creando el socket para IP:%s , PUERTO:%s \n", ip, puerto);
        return 0;
    }
    ...
    if(connect(socket_cliente, servinfo->ai_addr, servinfo->ai_addrlen) == -1) {
        ...
        log_info(logger, "Cliente conectado en IP:%s , PUERTO:%s, (a MODULO %s)\n", ip, puerto, server_name);
